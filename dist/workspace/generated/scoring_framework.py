...
def create_scoring_framework_doc():
    ...
    add_paragraph_custom(doc, 
        '核心目标：将主观经验转化为结构化评分标准，降低决策偏差，提高选股效率。')
    ...
    p_data = [
        ['P5', '国家级战略/重大突发事件', '如国家战略新兴产业规划、突发地缘冲突、重大技术突破', '5分'],
        ['P4', '部委级政策/行业重大变革', '如工信部专项支持、证监会改革、行业准入政策调整', '4分'],
        ['P3', '地方级重点政策/产业联盟', '如地方重点产业规划、央企重组、头部企业战略合作', '3分'],
        ['P2', '企业级重大公告/订单', '如重大合同签订、并购重组、核心技术突破公告', '2分'],
        ['P1', '一般性利好/常规公告', '如季度业绩预增、普通合作协议、日常经营利好', '1分']
    ]
    ...
    add_paragraph_custom(doc, '事件重要性得分 E = P × 0.35 + M × 0.35 + R × 0.30')
    ...
    add_paragraph_custom(doc, '稀缺性得分 S = U × 0.40 + I × 0.35 + C × 0.25')
    ...
    add_paragraph_custom(doc, 'TS = (E / 5) × 40% + (S / 5) × 60%', bold=True)
    ...
    add_paragraph_custom(doc, '┌─────────────────────────────────────────────────────────┐')
    add_paragraph_custom(doc, '│                    评分计算流程                          │')
    add_paragraph_custom(doc, '└─────────────────────────────────────────────────────────┘')
    add_paragraph_custom(doc, '                           │')
    add_paragraph_custom(doc, '           ┌──────────────┴──────────────┐')
    add_paragraph_custom(doc, '           ▼                             ▼')
    add_paragraph_custom(doc, '    ┌─────────────┐               ┌─────────────┐')
    add_paragraph_custom(doc, '    │ 事件重要性E  │               │   稀缺性S    │')
    add_paragraph_custom(doc, '    │   (40%)     │               │   (60%)     │')
    add_paragraph_custom(doc, '    └──────┬──────┘               └──────┬──────┘')
    add_paragraph_custom(doc, '           │                             │')
    add_paragraph_custom(doc, '    ┌──────┴──────┐               ┌──────┴──────┐')
    add_paragraph_custom(doc, '    ▼      ▼      ▼               ▼      ▼      ▼')
    add_paragraph_custom(doc, '   政策   预期   业绩            独特   资金   筹码')
    add_paragraph_custom(doc, '   级别   差     弹性            性     辨识度 结构')
    add_paragraph_custom(doc, '    │      │      │               │      │      │')
    add_paragraph_custom(doc, '    └──────┴──────┘               └──────┴──────┘')
    add_paragraph_custom(doc, '           │                             │')
    add_paragraph_custom(doc, '           │    E = Σ(维度×权重)         │    S = Σ(维度×权重)')
    add_paragraph_custom(doc, '           │      = P×0.35+M×0.35+R×0.30 │      = U×0.40+I×0.35+C×0.25')
    add_paragraph_custom(doc, '           │                             │')
    add_paragraph_custom(doc, '           └─────────────┬───────────────┘')
    add_paragraph_custom(doc, '                         ▼')
    add_paragraph_custom(doc, '              ┌───────────────────┐')
    add_paragraph_custom(doc, '              │ TS = (E/5)×0.4   │')
    add_paragraph_custom(doc, '              │      +(S/5)×0.6   │')
    add_paragraph_custom(doc, '              └─────────┬─────────┘')
    add_paragraph_custom(doc, '                        ▼')
    add_paragraph_custom(doc, '              ┌───────────────────┐')
    add_paragraph_custom(doc, '              │  映射到1-10分区间  │')
    add_paragraph_custom(doc, '              └─────────┬─────────┘')
    add_paragraph_custom(doc, '                        ▼')
    add_paragraph_custom(doc, '              ┌───────────────────┐')
    add_paragraph_custom(doc, '              │   输出最终评分     │')
    add_paragraph_custom(doc, '              │   附概率定性说明   │')
    add_paragraph_custom(doc, '              └───────────────────┘')
    ...
    add_paragraph_custom(doc, '映射公式：Final Score = 2 + TS × 8', bold=True)
    add_paragraph_custom(doc, '（当TS=0.2时得2分，TS=1时得10分，中间线性插值）')
    
    add_heading_custom(doc, '5.1 评分区间详解', level=2)
    
    table7 = doc.add_table(rows=11, cols=5)
    table7.style = 'Table Grid'
    
    headers7 = ['最终评分', 'TS范围', '场景描述', '多次涨停概率定性', '投资建议']
    ...
    add_paragraph_custom(doc, '场景假设：某低市值芯片设计公司X，国家发布国产算力重大扶持政策')
    
    # 创建评分计算展示表格
    ex1_table = doc.add_table(rows=8, cols=4)
    ...
    ex1_data = [
        ['事件重要性E(40%)', '政策级别P', '国家级战略政策', 'P5=5分'],
        ['', '预期差M', '政策力度超预期', 'M4=4分'],
        ['', '业绩弹性R', '订单预期翻倍', 'R4=4分'],
        ['', 'E计算', '5×0.35+4×0.35+4×0.30', 'E=4.35'],
        ['稀缺性S(60%)', '独特性U', '细分赛道唯一标的', 'U5=5分'],
    ...
    add_paragraph_custom(doc, 
        '最终得分：2 + 0.906×8 = 9.25 ≈ 9分（很高概率多次涨停）', bold=True)
    
    add_heading_custom(doc, '示例二：边缘沾边型（低分场景）', level=2)
    add_paragraph_custom(doc, '场景假设：某传统制造业公司Y，宣布通过子公司间接参与新能源')
    
    ex2_table = doc.add_table(rows=8, cols=4)
    ...
    add_paragraph_custom(doc, 
        '最终得分：2 + 0.258×8 = 4.06 ≈ 4分（多次涨停概率较低）', bold=True)
